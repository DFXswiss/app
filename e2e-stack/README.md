...
job after Build and test; it starts only after environment `pr-ci` is
approved on Build and test. GitHub may also hold fork runs as
`action_required`. Ready does not start CI. After a fresh A38 enforce
pass, `dfx pr guard` approves those waiting initial runs. A develop PR
without `ci:full` records `mode=none` and does
