...
| Customer-created RealUnit **buy quote** via `/buy`               | Full buy→quote path needs live pricing / payment-info services that fail under `loc` mocks. Specs that need a `WaitingForPayment` quote **SQL-insert** a `transaction_request` row instead. That proves admin list/detail/deactivate against the real API, **not** that a customer created the quote through `/buy`. |
