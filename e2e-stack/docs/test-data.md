...
| Real Lightning/BTC **payment-link quote**                        | `createPayRequest` 404s with "No BTC transfer amount found". The two device-split `/pl` tests therefore fulfill `paymentLink/payment` themselves — see `docs/test-reality.md`. A green run does not prove the loc API can quote a payment.    |
