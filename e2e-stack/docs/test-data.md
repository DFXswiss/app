...
| **Returns**                         | `{ buyId, routeId?, assetId?, requestId?, iban?, remittanceInfo? }` (`requestId` / `iban` / `remittanceInfo` only when `withPaymentInfo` is true)                                                           |
...
| **Returns** | `{ transactionId?, uid, buyCryptoId?, buyFiatId?, bankTxId?, cryptoInputId?, buyId?, sellId?, userId?, userDataId?, iban?, remittanceInfo? }` (`transactionId` absent for `waiting_for_payment_buy`; `iban` / `remittanceInfo` only for that state)                                                               |
