...
| **Returns**                         | `{ buyId, routeId?, assetId?, requestId?, remittanceInfo? }` (`requestId` / `remittanceInfo` only when `withPaymentInfo` is true)                                                                           |
...
| **Returns** | `{ transactionId?, uid, buyCryptoId?, buyFiatId?, bankTxId?, cryptoInputId?, buyId?, sellId?, userId?, userDataId?, remittanceInfo? }` (`transactionId` absent for `waiting_for_payment_buy`; `remittanceInfo` only for that state)                                                                               |
...
|               |                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Path**      | SQL on `user_data."phoneCallStatus"` and `"phoneCallStatusDate"` (+ optional pending `buy_crypto` via `createTransaction`); against an API without the date column (before DFXswiss/backend#5614) the status alone                                                                                                                                                                                                                       |
| **Returns**   | `{ userDataId, phoneCallStatus?, transactionId?, buyCryptoId? }`                                                                                                                                                                                                                                                                                                                                                                         |
| **Important** | **There is no `call_queue` table.** Support dashboard queues are derived (`support.service.ts`) and list transactions: the reason queues the pending txs with AML reasons such as `ManualCheckPhone`, `ManualCheckIpPhone`, etc.; the Callback queue (key `UnavailableSuspicious`) the phone txs created on or before the `phoneCallStatusDate` of an `Unavailable` account, and the failed phone txs of a `UserRevokeDecision` account. |
...
| Live **price-based** payment infos                               | Outbound HTTP is mocked. `global.setup.ts` backfills `price_rule` timestamps so `PUT /buy/paymentInfos` can complete from seed prices; without that backfill the path hangs or fails. Customer `WaitingForPayment` bank buys use this path (`waiting_for_payment_buy`). |
...
| Customer-created RealUnit **buy quote** via `/buy`               | RealUnit admin quote screens still SQL-insert a `transaction_request` for a REALU token quote. That is not the customer bank-buy path: a `WaitingForPayment` CHF/EUR buy is created through `PUT /buy/paymentInfos` plus confirm (`waiting_for_payment_buy`).           |
