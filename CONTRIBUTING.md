...
Draft pull requests run the GitHub PR CI jobs. Those jobs wait on environment
`pr-ci` until a required reviewer or `dfx pr guard` approves. GitHub also holds
fork runs from external contributors as `action_required`. Ready does not start
CI. After a
...
Draft pull requests run the PR CI jobs. Those jobs wait on environment `pr-ci`
until a required reviewer or `dfx pr guard` approves. GitHub also holds fork
runs from external contributors as `action_required`. Ready does not start CI.
After a fresh A38
enforce pass on
...
and test succeed. Draft pull requests run the job. The job waits on environment
`pr-ci` until a required reviewer or `dfx pr guard` approves. GitHub also holds
fork runs from external contributors as `action_required`. Ready does not start
CI. After a
fresh A38 enforce pass on
