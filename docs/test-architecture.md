...
- **The 2FA merged-account redirect spec answers the 2FA setup call and its post-redirect
  ...
  bootstrap GETs (`/v1/language`, `/v1/fiat`, `/v1/asset`, `/v1/bankAccount`, `/v1/country`,
  `/v1/setting/infoBanner`) and `POST /v1/log/clientError`. Unmatched `/v1/**` and `/v2/**`
  calls get `501`. A green run proves the screen redirects on that specific error shape. It
  does not prove that the API ever returns a 401 with `switchToCode` for a merged account, or
...
- **The link merged-account redirect spec answers GET /v2/user and GET /v2/kyc itself.**
  ...
  follow-up call after the redirect, plus the same bootstrap GETs (`/v1/language`, `/v1/fiat`,
  `/v1/asset`, `/v1/bankAccount`, `/v1/country`, `/v1/setting/infoBanner`) and
  `POST /v1/log/clientError`. Unmatched `/v1/**` and `/v2/**` calls get `501`. A green run
  proves the screen redirects on that specific error shape. It does not prove that login or
