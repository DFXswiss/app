...
- **The RealUnit quotes and dashboard visual specs answer the admin list themselves.**
  ...
  A green run proves the screens render those fixtures, not that the API returns them, not that
  login or token verification works, and not that those staff/settings or stats endpoints return real
  payloads.
