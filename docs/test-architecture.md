...
- **The 2FA merged-account redirect spec answers the 2FA setup call and its post-redirect
  ...
  proves the screen redirects on that specific error shape. It does not prove that the API
  ever returns a 401 with `switchToCode` for a merged account, or that the `/kyc` screen's own
